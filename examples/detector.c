		    ...
		    int *gpus, int ngpus, int clear)
{
    ...
    char *train_images =
	option_find_str(options, "train", "data/train.list");
			    ...
			    char *outfile)
{
    ...
    list *options = read_data_cfg(datacfg);
    char *valid_images =
	option_find_str(options, "valid", "data/train.list");
	...
	for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
	    ...
	    copy_cpu(net->w * net->h * net->c,
		     val_resized[t].data, 1, input.data, 1);
	    flip_image(val_resized[t]);
	    copy_cpu(net->w * net->h * net->c,
		     val_resized[t].data, 1,
	    ...
	    int h = val[t].h;
	    int num = 0;
	    detection *dets = get_network_boxes(net, w, h, thresh, .5, map,
						0, &num);
	    if (nms)
		...
		print_imagenet_detections(fp,
					  i + t -
					  nthreads + 1,
					  dets, num, classes, w, h);
	    } else {
		print_detector_detections(fps, id, dets,
					  num, classes, w, h);
	    }
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
	...
	int nboxes = 0;
	detection *dets =
	    get_network_boxes(net, sized.w, sized.h, thresh, .5, 0,
			      1, &nboxes);
	if (nms)
...
void test_detector_image(char *filename, char *outfile, network * net,
			 float thresh, float hier_thresh, char **names,
			 image ** alphabet)
{
    double time;
    char buff[256];
	...
	int nboxes = 0;
	detection *dets =
	    get_network_boxes(net, im.w, im.h, thresh, hier_thresh,
			      0, 1, &nboxes);
	//printf("%d\n", nboxes);
	...
	if (nms)
        ...
        //printf("--------------------------");
	draw_detections(im, dets, nboxes, thresh, names, alphabet,
			l.classes);
	free_detections(dets, nboxes);
	if (outfile) {
	    ...
	    struct stat st;
	    stat(outfile, &st);
            //get last save result filename. example:(smoke1_out.jpg)
		...
		char *filesimpleName;
		int i = 0;
		for (token = strsep(&tempFileName, "/"); token != NULL;
		     token = strsep(&tempFileName, "/")) {
		    i++;
		    if (i == 2) {
			//printf("token  %s\n", token);
			filesimpleName = strsep(&token, ".");
			if (filesimpleName != NULL) {
			    memset(outfilepath, 0, sizeof(outfilepath));
                            strcat(outfilepath, outfile);
			    strcat(outfilepath, "/");
                            strcat(outfilepath, filesimpleName);
                            strcat(outfilepath, "_out");
			}
			//printf("outfile  %s\n", outfilepath);
			break;
		    }
            ...
            printf("outfile: %s\n\n", outfilepath);
	    save_image(im, outfilepath);
            memset(outfilepath,0x0,255);
	} else {
		   ...
		   char *outfile, int fullscreen)
{
    ...
    printf("image name or dir: %s \n", filename);
    printf("out file name or dir: %s \n", outfile);
	...
	while ((entry = readdir(dir)) != 0) {
		...
		printf("-------detect %s-------\n", buff);
                test_detector_image(buff, outfile, net, thresh, hier_thresh, names,
			alphabet);
	    }
	...
	closedir(dir);
     ...
     printf("-------detect %s-------\n", filename);
     test_detector_image(filename, outfile, net, thresh, hier_thresh, names,
			alphabet);
     }
...
void run_detector(int argc, char **argv)
{
	...
	char *name_list =
	    option_find_str(options, "names", "data/names.list");
	char **names = get_labels(name_list);
	demo(cfg, weights, thresh, cam_index, filename, names,
	     classes, frame_skip, prefix, avg, hier_thresh, width,
